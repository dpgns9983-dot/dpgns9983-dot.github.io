# HDC AI Portfolio Site

HDC AI 과정에서 진행한 프로젝트를 면접용으로 정리한 포트폴리오 웹사이트입니다.

배포 주소: https://dpgns9983-dot.github.io/hdc-ai-portfolio-site/

## 구성

- 30초 자기소개
- 학습 흐름
- 대표 프로젝트 2개
- 미니 프로젝트 4개
- 프로젝트별 문제 해결 STAR 분해
- GitHub / Notion / PDF / Email 연결 영역

## 포트폴리오 방향

이 사이트는 `AI 서비스 개발자 지망생`이라는 방향으로 작성했습니다.

프로젝트 설명은 단순히 사용 기술을 나열하지 않고, 아래 흐름으로 정리합니다.

- 어떤 문제가 있었는지
- 내가 맡은 역할은 무엇인지
- 어떤 행동으로 해결했는지
- 결과물은 무엇인지
- 면접에서 과장하면 안 되는 부분은 무엇인지

## 프로젝트 목록

1. 서울 범죄 통계 대시보드
2. Smart Mirror AIoT
3. 미세먼지와 서울 야외활동 분석
4. 호텔 프로젝트
5. 구독관리 에이전트
6. 커피 원두 품질/로스팅 판별

## 로컬 실행

```powershell
npm install
npm run dev
```

## 빌드

```powershell
npm run build
```

## GitHub Pages 배포

이 저장소는 GitHub Actions로 `main` 브랜치에 push될 때 자동 배포되도록 설정되어 있습니다.

GitHub repository의 `Settings > Pages`에서 `Source`가 `GitHub Actions`로 설정되어 있어야 합니다.

## 공개 전 확인

- PDF 링크와 이메일을 실제 정보로 교체하기
- API 키, 개인정보, 강의 원본 자료가 포함되지 않았는지 확인하기
- 프로젝트 이미지가 공개 가능한 자료인지 확인하기
- 원본 프로젝트의 대용량 데이터와 민감 파일은 공개 저장소에 직접 올리지 않기
