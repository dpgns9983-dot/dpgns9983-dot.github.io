...
const careerCards = [
  ...
  },
  {
    ...
    body: "AI 분석 결과와 코칭 내용을 사용자가 따라갈 수 있는 화면 흐름과 서비스 시나리오로 연결",
    icon: BrainCircuit,
  },
  {
    title: "최종 프로젝트",
...
const projects = [
  {
    ...
    type: "Data Dashboard",
    period: "2026",
    role: "요일별 범죄 데이터 전처리, 핫스팟-자치구 매핑 구조 정리, Streamlit 분석 흐름 구성",
    outcome: "공공 범죄 통계와 실시간 도시데이터를 지역, 시간, 요일, 핫스팟 기준으로 탐색하는 대시보드",
    problem: "경찰청 범죄 통계는 자치구 기준, 서울시 실시간 도시데이터는 핫스팟 장소 기준이라 바로 결합하기 어려운 구조",
    implementation: "전처리 CSV 정리, mapping_fix.csv 기반 핫스팟-자치구 연결, SQLite/SQLAlchemy ORM 적재, Streamlit 5개 페이지 구성",
    result: "홈, 실시간 핫스팟, 지역, 시간대, 요일 분석으로 이어지는 SafeNOVA 대시보드 흐름",
    artifacts: "전처리 노트북, police_week_fix.csv, mapping_fix.csv, seed_all.py, ORM 모델, Streamlit 분석 화면",
    lesson: "데이터 분석 결과도 DB 구조와 화면 흐름으로 연결되어야 서비스로 전달된다는 점 확인",
    evidence: [
      ...
      "police_region_fix.csv 234행, police_time_fix.csv 72행, police_week_fix.csv 63행, mapping_fix.csv 123행 기반",
    ],
  ...
  },
  {
    ...
    problem: "PC1 화면, PC3 자세 분석, PC2 AI 코칭 기능이 각각 구현됐지만 합쳤을 때 사용자가 결과와 코칭 문구를 확인하는 흐름이 분리될 수 있는 상황",
    implementation: "React/Tauri 기반 프로필 선택, 기준 촬영, 운동 세션, 결과 확인 화면 구성, 목데이터 기반 카운트와 코칭 문구 검증, 결과 수신 재시도 흐름 정리",
